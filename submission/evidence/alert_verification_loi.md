...
## Alert 1 — HighLatencyP95 (`rag_slow`)
...
Kết quả: p95 = 3540ms > ngưỡng 3000ms → điều kiện `latency_p95_ms > 3000 for 5m` trong `config/alert_rules.yaml` sẽ khớp. Runbook: `docs/alerts.md#alert-1`.

## Alert 2 — ElevatedErrorRate (`tool_fail`)
...
Kết quả: 100% request trong batch lỗi (RuntimeError) → vượt xa ngưỡng `error_rate_pct > 2 for 5m`. Runbook: `docs/alerts.md#alert-2`.
...
## Alert 3 — CostBudgetBurn (`cost_spike`)
...
Kết quả: cost/request tăng ~4.6 lần so với baseline → vượt điều kiện `cost_rate_per_minute > 2x baseline for 10m`. Runbook: `docs/alerts.md#alert-3`.
